- 0.1.0 : modification du choix de l'interface graphique de PySide6 à Django


- 0.0.6 : ajout les fichiers d'installation
20260119 : ajout de l'icone et génération du fichier desktop
    + création du install.sh
20260120 : correctif pour l'execution du programme dans le conteneur

- 0.0.5 : maintenance
20260106 : correctif sur le bouton "Utiliser le répertoire courant"
20260107 : creation d'un conteneur Docker

- 0.0.4 : gestion du crash sur pc du bas
20260105: ajout de logs + bouton alternatif "Utiliser le répertoire courant" 
    + rappelle à l'utilisateur qu'il peut saisir manuellement le chemin

- 0.0.3 : gestion du crash sur pc du bas
20260103: ajout de la gestion des exceptions
20260104: ajout d'un log de debug et correction du crash en fin de traitement

- 0.0.2 : première version fonctionnelle
20251224: generer un fichier zip correspondant à l'execution avec date et heure
20251226: ajout d'une interface graphique
20251231: modification du choix de l'interface graphique de tkinter à PySide6

- 0.0.1 : première version