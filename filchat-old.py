...
class MainWindow(QMainWindow):
    ...
    def log(self, message):
        try:
            ...
            logger.error(f"Erreur dans log: {str(e)}")
    ...
    def on_thread_finished(self):
        ...
        try:
            logger.info("Thread terminé et nettoyé")
            if self.worker:
                self.worker.deleteLater()
                self.worker = None
            # logger.info("worker supprimé")
            if self.worker_thread:
                self.worker_thread.deleteLater()
                self.worker_thread = None
            # logger.info("worker_thread supprimé")
        except Exception as e:
            logger.error(f"Erreur dans on_thread_finished: {str(e)}")

    def show_error(self, message):
        try:
            logger.error(f"Affichage erreur: {message}")
...
class TkLogHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            pass  # Ne pas planter si l'interface n'est plus disponible
